...
def print_data_exploration(client_data):
...
def print_data_exploration_categorical(client_data):
...
def create_Nan_indicator_column(dataframe, column, new_column_name, impute, imputed_value):
    """Create new column, populate 1/0 for missing/exists. Impute NaN with new value"""
...
def data_repair_and_manipulation(data):
    # Handle the special time field
    data['time'] = (pd.to_datetime(data['pit']))
    minimum_date = data['time'].min()
    data['time'] = ((data['time'] - minimum_date) / np.timedelta64(1, 'D')).astype(
        int)  # Count in days since minimum date
    ...
    for fill_info in nan_indicator_new_columns:
        data = create_Nan_indicator_column(data, fill_info[0], fill_info[1], fill_info[2], fill_info[3])
    ...
    data.replace(categorical_replacements, inplace=True)
    data = add_years_boolean(data)
    return data
...
def train_and_predict(df, dropped_columns):
    ...
    df = df_sorted.drop_duplicates(['client_id'], keep='last')
...
def main():
    ...
    print_data_exploration(client_data)
    print("\r\nBefore categorical encoding\r\n------------------------")
    print_data_exploration_categorical(client_data)

    client_data = data_repair_and_manipulation(client_data)
